...
btnApplyBottom &&
  (btnApplyBottom.onclick = () => {
    ...
    if (DB.me.cls !== old) {
      addNotif(
        DB.lang === 'zh' ? `切換年級：${DB.me.cls}` : `Grade -> ${DB.me.cls}`
      );
    }
...
// ===== Render (略: 保持與之前一致) =====
// renderTop, renderMeta, renderSkills, renderProfileSkills, renderTasks, renderSide, updateAll, renderNotifs, drawRadar
// ⚠️ 這些函數你保留原本的即可，不要刪掉。
...
btnApplyAvatar &&
  (btnApplyAvatar.onclick = () => {
    if (DB.me.avatarImg) {
      addNotif(DB.lang === 'zh' ? '角色圖片已套用' : 'Avatar applied');
      applyAvatar();
    } else {
      addNotif(DB.lang === 'zh' ? '尚未選擇圖片' : 'No image selected');
    }
  });
btnClearAvatar &&
